...
### 1. 🏠 Welcome & Authentication (`welcome.html`, `signup.html`, `signin.html`)
...
### 3. 💳 Transactions Management (`transactions.html`)
...
### 4. ⚠️ Anomaly Center & Risk Monitor (`anomalies.html`)
...
### 5. ▣ Smart Category Analysis (`categories.html`)
...
### 6. ◫ Budget Limits & Health (`budgets.html`)
...
### 7. ▤ Spending Intelligence Reports (`reports.html`)
...
### 9. ⚙️ Control Center & Settings (`settings.html`)
...
- **$\mu$**: Mean (average) spending for transactions in that specific category:
  $$\mu = \frac{\sum_{i=1}^N X_i}{N}$$
- **$\sigma$**: Standard deviation of spending in that category:
  $$\sigma = \sqrt{\frac{\sum_{i=1}^N (X_i - \mu)^2}{N}}$$

### Classification Criteria:
- **Normal**: $|Z| < 1.8$ (Within expected statistical distribution)
- **Medium Risk**: $1.8 \le |Z| < 2.6$ or single-item fallback ratio $\ge 1.6\times$ global mean.
- **High Risk**: $|Z| \ge 2.6$ or amount $\ge 3\times$ category mean.
...
ExpenseGuard is designed as a standalone client-side web application leveraging browser `localStorage` and `sessionStorage`:

| Storage Key | Description |
| :--- | :--- |
| `expenseGuardUsers` | Array of registered user accounts (`name`, `email`, `password`) |
| `expenseGuardCurrentUser` | Currently authenticated user object (`name`, `email`) |
| `expenseGuardLoggedIn` | Boolean string (`"true"` / `"false"`) indicating active session |
| `expenseGuardTransactions` | Array of transaction objects (`id`, `date`, `category`, `description`, `amount`) |
| `expenseGuardBudgets` | Key-value dictionary of category monthly limits |
| `expenseGuardTheme` | Current theme selection (`"navy"` or `"light"`) |
| `expenseGuardSetting_*` | User preferences (currency, default category, week start, toggles) |
